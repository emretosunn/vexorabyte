    ...
    <main className="relative mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-12 px-6 py-12 md:px-10">
      ...
      <header className="fixed inset-x-0 top-4 z-30 mx-auto flex w-[calc(100%-2rem)] max-w-6xl flex-wrap items-center justify-between gap-3 rounded-2xl border border-zinc-700/60 bg-zinc-900/35 px-4 py-3 shadow-[0_8px_30px_rgba(0,0,0,0.35)] backdrop-blur-xl md:px-8">
        <div className="flex w-full items-center gap-2 sm:w-auto sm:gap-3">
          <Link href={`/?lang=${locale}#apps`} className="rounded-full border border-zinc-700 px-3 py-2 text-xs text-zinc-200 transition hover:border-zinc-500 hover:text-zinc-100 sm:px-4 sm:text-sm">
            {copy.home}
          </Link>
          <Link href={`/?lang=${locale}#careers`} className="rounded-full border border-zinc-700 px-3 py-2 text-xs text-zinc-200 transition hover:border-zinc-500 hover:text-zinc-100 sm:px-4 sm:text-sm">
            {copy.careers}
          </Link>
        </div>
